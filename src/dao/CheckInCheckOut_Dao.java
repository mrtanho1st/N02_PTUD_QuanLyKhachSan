...
public class CheckInCheckOut_Dao {
	...
	public boolean capNhatNgayTra(String maDDP, String ngayTraMoi) {
		if (!isNgayTraHopLe(maDDP, ngayTraMoi)) {
			return false;
		}
	...
	private boolean isNgayTraHopLe(String maDDP, String ngayTraMoi) {
		...
		String ngayNhanHienTai = null;
		String sql = """
				SELECT CONVERT(VARCHAR, ngayNhan, 120) AS ngayNhan
				FROM DonDatPhong
				WHERE maDDP = ?
				""";
		...
		try (
			...
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next()) {
					ngayNhanHienTai = rs.getString("ngayNhan");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		LocalDateTime start = parseNgayGio(ngayNhanHienTai);
		LocalDateTime end = parseNgayGio(ngayTraMoi);
